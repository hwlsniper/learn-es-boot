...
import com.learn.es.repository.SubwayRepository;
import com.learn.es.repository.SubwayStationRepository;
...
import com.learn.es.service.search.BaiduMapLocation;
import com.learn.es.web.dto.SubwayDTO;
import com.learn.es.web.dto.SubwayStationDTO;
import com.learn.es.web.dto.SupportAddressDTO;
...
public class AddressServiceImpl implements IAddressService {
    ...
    public ServiceResult<BaiduMapLocation> getBaiduMapLocation(String city, String address) {
        ...
        try {
            ...
            log.error("方法:{};错误信息:{};异常信息:{}", AddressServiceImpl.class.getName()+".getBaiduMapLocation()", "Error to encode house address!", e.getMessage());
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                log.error("方法:{};错误信息:{}", AddressServiceImpl.class.getName()+".getBaiduMapLocation()", "Can not get baidu map location!");
            ...
            if (status != 0) {
                log.error("方法:{};错误信息:{}", AddressServiceImpl.class.getName()+".getBaiduMapLocation()", "Error to get map location for status: " + status);
            ...
            log.error("方法:{};错误信息:{};异常信息:{}", AddressServiceImpl.class.getName()+".getBaiduMapLocation()", "Error to fetch baidumap api!", e.getMessage());
    ...
    public ServiceResult lbsUpload(BaiduMapLocation location, String title,
        ...
        try {
            ...
            String result = EntityUtils.toString(response.getEntity(), "UTF-8");
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                log.error("方法:{};错误信息:{}", AddressServiceImpl.class.getName()+".lbsUpload()", "Can not upload lbs data for response: " + result);
                ...
                if (status != 0) {
                    ...
                    log.error("方法:{};Error to upload lbs data for status: {}, and message: {}", AddressServiceImpl.class.getName()+".lbsUpload()", status, message);
    ...
    private boolean isLbsDataExists(Long houseId) {
        ...
        try {
            ...
            String result = EntityUtils.toString(response.getEntity(), "UTF-8");
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                log.error("方法:{};错误信息:{}", AddressServiceImpl.class.getName()+".isLbsDataExists()", "Can not get lbs data for response: " + result);
                return false;
            }
            ...
            int status = jsonNode.get("status").asInt();
            if (status != 0) {
                log.error("方法:{};错误信息:{}", AddressServiceImpl.class.getName()+".isLbsDataExists()", "Error to get lbs data for status: " + status);
                return false;
            } else {
    ...
    public ServiceResult removeLbs(Long houseId) {
        ...
        try {
            ...
            String result = EntityUtils.toString(response.getEntity(), "UTF-8");
            if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                log.error("方法:{};错误信息:{}", AddressServiceImpl.class.getName()+".removeLbs()", "Error to delete lbs data for response: " + result);
                return new ServiceResult(false);
            }
            ...
            if (status != 0) {
                String message = jsonNode.get("message").asText();
                log.error("方法:{};错误信息:{}", AddressServiceImpl.class.getName()+".removeLbs()", "Error to delete lbs data for message: " + message);
                return new ServiceResult(false, "Error to delete lbs data for: " + message);
            }
            return ServiceResult.success();
        } catch (IOException e) {
            log.error("方法:{};错误信息:{};异常信息:{}", AddressServiceImpl.class.getName()+".removeLbs()", "Error to delete lbs data.", e.getMessage());
            return new ServiceResult(false);
        }
    }
}

package com.learn.es.service.sms;
...
import com.learn.es.service.user.UserServiceImpl;
 ...
 * @author he.wl
 * @version 1.0.0
 * @className com.learn.es.service.sms.SmsServiceImpl
 * @description 短信发送服务
...
public class SmsServiceImpl implements ISmsService, InitializingBean {
	...
	public ServiceResult<String> sendSms(String telephone) {
		...
		if (result != null) {
			log.error("方法{};错误信息:{}", SmsServiceImpl.class.getName()+"sendSms()", "请求次数太频繁");
		...
		try {
			...
			if ("OK".equals(response.getCode())) {
				...
				log.error("方法:{},错误码:{},错误信息:{}", SmsServiceImpl.class.getName()+".sendSms()", response.getCode(), response.getMessage());
			}
		} catch (ClientException e) {
			log.error("方法{};错误信息:{}", SmsServiceImpl.class.getName()+"sendSms()", e.getMessage());
			e.printStackTrace();
		}
		if (success) {
			...
			log.error("方法{};错误信息:{}", SmsServiceImpl.class.getName()+"sendSms()", "服务忙，请稍后重试");
